...
def Calc_groundtruth_HB(value_buff, time_buff, sensor_type):
    if sensor_type == "ppg":
        peak = np.array(find_peaks(value_buff, height=np.average(value_buff)+5, distance=40))
    elif sensor_type =="ecg":
        value_buff = np.array(value_buff)
        b_filt_heart,a_filt_heart =filter_adjust("highpass")
        value_buff = (signal.filtfilt(b_filt_heart, a_filt_heart, value_buff))
        peak = np.array(find_peaks(value_buff, height=75, distance=15),dtype=list)
        np.seterr(invalid="ignore")
    elif sensor_type == "gdx":
        value_buff = np.array(value_buff)
        b_filt_heart, a_filt_heart = filter_adjust("highpass")
        value_buff = (signal.filtfilt(b_filt_heart, a_filt_heart, value_buff))
        peak = np.array(find_peaks(value_buff, height=0.4, distance=80),dtype=list)
        np.seterr(invalid="ignore")
