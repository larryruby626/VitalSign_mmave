...
if __name__ == "__main__":
    ...
    if process_type == "SICON":
        noise_floor = np.load("./noise_floor/sumof_noise_floor_sicON.npy", allow_pickle=True)
        if channel == 1:
            path = "./dataset_7G_SIC_ON/processed_data/channel1/"  # KKT usage path
        elif channel == 2:
            path = "./dataset_7G_SIC_ON/processed_data/channel2/"  # KKT usage path

    noise_index = list(noise_floor[0,:])

    #  ---- for chest data usage ----
    angle_list = ["-40", "-30", "-20", "-10", "0", "+10", "+20", "+30", "+40"]
    ...
    if sure_xlsx:
        ...
        if channel ==1 :
            workbook = xlsxwriter.Workbook("./out_result/SNR_SIC_ON_CH1.xlsx")
        elif channel ==2:
            workbook = xlsxwriter.Workbook("./out_result/SNR_SIC_ON_CH2.xlsx")
        ...
        for method in range(1,3):
            for time in range(1,2):
                ...
                worksheet.write(row, 1, "angle-40")  # Writes a float
                worksheet.write(row, 2, "angle-30")  # Writes a float
                worksheet.write(row, 3, "angle-20")  # Writes a float
                worksheet.write(row, 4, "angle-10")  # Writes a float
                worksheet.write(row, 5, "angle0")  # Writes a float
                worksheet.write(row, 6, "angle+10")  # Writes a float
                worksheet.write(row, 7, "angle+20")  # Writes a float
                worksheet.write(row, 8, "angle+30")  # Writes a float
                worksheet.write(row, 9, "angle+40")  # Writes a float
    ...
    if channel ==1 :
        save_obj(SNR_dic, "SNR_SICON_DIC_ch1")
    elif channel ==2:
        save_obj(SNR_dic, "SNR_SICON_DIC_ch2")
