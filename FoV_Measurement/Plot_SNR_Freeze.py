...
def plot_Fov_single(input_fov_data, title):
    plt.figure()
    plt.imshow(input_fov_data, interpolation="bilinear", cmap=cm.jet, vmin=0, vmax=55)
    plt.title(str(title))
    plt.colorbar()
...
def plot_Fov(input_fov_data, subplot_cc, plot_row_len, plot_col_len, row, col, title):
    methods = ['bilinear']
    if subplot_cc == 0:
        fig, axs = plt.subplots(nrows=pslot_row_len, ncols=plot_col_len, figsize=(9, 6),
                                subplot_kw={'xticks': [], 'yticks': []})
...
for method in range(1, 3):
    for channel in range(1, 3):
        r = 0
        tmp_time1 = np.zeros([11, 9])
        snr_dic = load_obj("SNR_Freeze_DIC_ch" + str(channel))
        for rangeX in range_list:
            ...
            r += 1
        if method == 1:
            title = "method: " + "spline" + ", channel:" + str(channel + 1)
        elif method == 2:
            title = "method: " + "quadratic" + ", channel:" + str(channel + 1)
